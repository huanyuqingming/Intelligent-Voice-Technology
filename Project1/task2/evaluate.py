...
def get_metrics(prediction, label, threshold=0.5, plot=False):
    ...
    prediction_labels = [1 if pred >= threshold else 0 for pred in prediction]
    acc = metrics.accuracy_score(label, prediction_labels)
